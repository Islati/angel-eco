...
import io.gridmc.angel.api.commands.handlers.*;
import org.bukkit.*;
...
public class CommandHandler implements CommandExecutor {
        ...
        for (Method method : commands.getClass().getDeclaredMethods()) {
            ...
            if (rootPcommand == null) {
                if (!commands.getClass().isAnnotationPresent(CommandInfo.class)) {
                    ...
                    Angel.getInstance().getLogger().info("Registering commands in " + commands.getClass().getSimpleName());
    ...
    private void registerCommandInfo(CommandInfo info, PluginCommand cmd) {
        ...
        Bukkit.getConsoleSender().sendMessage("&7Command &e/" + info.name() + "&7 registered.");
    ...
    public boolean onCommand(CommandSender sender, org.bukkit.command.Command command, String label, String[] args) {
        ...
        if (rootCommand.onlyPlayers() && !(sender instanceof Player)) {
            sender.sendMessage("This message is only for players.");
            return true;
        }
